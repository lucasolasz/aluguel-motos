...
@Converter
@Component
public class CartaoNumeroEncryptor implements AttributeConverter<String, String> {

    private static volatile byte[] KEY_BYTES;

    @Value("${security.card-encryption.key}")
    public void setCardEncryptionKey(String rawKey) {
        if (rawKey == null || rawKey.isBlank()) {
            throw new IllegalStateException("CARD_ENCRYPTION_KEY must not be empty");
        }
        try {
            KEY_BYTES = MessageDigest.getInstance("SHA-256")
                    .digest(rawKey.getBytes(StandardCharsets.UTF_8));
        } catch (Exception e) {
            throw new IllegalStateException("Cannot derive AES key for card number", e);
        }
    }
    ...
    public String convertToEntityAttribute(String dbData) {
        ...
        if (parts.length != 2) return dbData;
        try {
